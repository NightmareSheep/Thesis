...
namespace BasicTreedepthSolver
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string filename = Environment.CurrentDirectory + "\\graph.txt";
            Graph g;

            using (StreamReader reader = File.OpenText(filename))
            {
                g = Graph.ParsePACE2016(reader);
            }
    ...
    public class Graph
    {
        ...
        public PartialSolution Treedepth()
        {
            sw.Start();
            for (UB = 1; UB <= Vertices.Length; UB++)
            {
                var solution = TrySolve();
                if (TrySolve() != null) return solution;
            }
            return null;
        }

        public PartialSolution TrySolve()
        {
    ...
    public class PartialSolution
    {
        ...
        public string[] SolutionPrint { get; set; }

        public void Print()
        {
            Vertex[] parents = new Vertex[G.Vertices.Length];
            SolutionPrint = new string[G.Vertices.Length];
            Print(parents, null);
            ...
            foreach (Vertex v in G.Vertices.OrderBy((v) => v.OriginalLabel))
            {
                ...
                {
                    Console.WriteLine("0");
                    SolutionPrint[v.OriginalLabel] = "0";
                }
                else
                {
                    Console.WriteLine((parent.OriginalLabel + 1));
                    SolutionPrint[v.OriginalLabel] = (parent.OriginalLabel + 1).ToString();
                }
            }
